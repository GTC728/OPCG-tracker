...
import { getChartSliceFill, getDeckIdentityFill, getDeckSliceFill, summarizeColorPreference } from '@/lib/deckChartColors'
...
export function sliceFillForDeckUsage(
  slice: DeckUsageSlice,
  index: number,
  slices: DeckUsageSlice[],
): string {
  if (slice.deckId === '__other__') return getChartSliceFill(index)
  return getDeckSliceFill(slice, index, slices)
}

function DeckUsageDonut({
  chartSlices,
  compact,
  inline,
}: {
  chartSlices: DeckUsageSlice[]
  compact: boolean
  inline: boolean
}) {
  const { t } = useI18n()
  const chartData = chartSlices.map((slice, index) => ({
    ...
    value: slice.count,
    fill: sliceFillForDeckUsage(slice, index, chartSlices),
    deckId: slice.deckId,
  }))

  const heightClass = inline ? 'h-[7.5rem] w-[7.5rem]' : compact ? 'h-28' : 'h-48'

  return (
    <div className={inline ? heightClass : `w-full ${heightClass}`}>
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            ...
            dataKey="value"
            nameKey="name"
            innerRadius={inline ? 24 : compact ? 28 : 42}
            outerRadius={inline ? 36 : compact ? 44 : 68}
            paddingAngle={2}
            stroke="color-mix(in srgb, var(--color-surface) 40%, transparent)"
            strokeWidth={2}
          >
            {chartData.map((entry) => (
              <Cell key={entry.deckId} fill={entry.fill} />
            ))}
          </Pie>
          <Tooltip
    ...
    <ul className="space-y-2">
      {slices.map((slice, index) => (
        <li key={slice.deckId} className="flex items-center gap-2 text-sm">
          <ChartLegendSwatch fill={sliceFillForDeckUsage(slice, index, slices)} />
...
export function DeckUsagePieChart({
  ...
  title: string
  compact?: boolean
  /** No outer card — for profile deck panel beside cards */
  inline?: boolean
}) {
  const { t } = useI18n()
  if (!slices.length) return null

  const chartSlices = compact && !inline ? collapseUsageSlices(slices, PREVIEW_SLICE_LIMIT) : slices
  const colorPref = summarizeColorPreference(slices)

  const body = (
    <>
      {!inline && !compact && title ? <h2 className={uiSectionTitle}>{title}</h2> : null}

      {!compact && !inline && colorPref.length > 1 ? (
        <div className="flex flex-wrap gap-2">
          {colorPref.map((item) => (
            ...
            >
              <ChartLegendSwatch metaColor={item.color} size="xs" />
      ...
      <DeckUsageDonut chartSlices={chartSlices} compact={compact} inline={inline} />
  ...
  return (
    <section className={[uiGlassCard, compact ? 'space-y-2 p-3' : 'space-y-3 p-4'].join(' ')}>
      {compact && title ? <p className="text-xs font-semibold text-text-primary">{title}</p> : null}
      {body}
    </section>
  )
}
...
export function buildDeckUsageFillMap(slices: DeckUsageSlice[]): Map<string, string> {
  ...
  slices.forEach((slice, index) => {
    map.set(slice.deckId, getDeckIdentityFill(slice, index, slices))
  })
  return map
}
