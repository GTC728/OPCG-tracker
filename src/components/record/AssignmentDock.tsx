...
import { getAssignmentRecentDeckIds, hasExplicitSessionRoster } from '@/lib/selectors'
import {
...
export function AssignmentDock({
  ...
  const addPlayer = useAppStore((store) => store.addPlayer)
  const setSessionRoster = useAppStore((store) => store.setSessionRoster)
  ...
  const recentDeckIds = useMemo(
    ...
    [activeMatches, decks, matches, players, sessionId],
  )
  ...
  const handleAddPlayer = () => {
    ...
    try {
      ...
      if (hasExplicitSessionRoster(state, sessionId)) {
        const rosterIds = state.sessionPlayers
          .filter((item) => item.sessionId === sessionId)
          .map((item) => item.playerId)
        setSessionRoster(sessionId, [...new Set([...rosterIds, player.id])])
      }
      setNewPlayerName('')
    ...
    <section className="rounded-xl bg-surface-elevated p-3">
      ...
      <div className="mt-2">
        ...
        <div className="mt-2 flex gap-2">
          <input
            className="min-h-9 flex-1 rounded-lg border border-surface-muted bg-surface px-3 py-1.5 text-sm text-text-primary outline-none focus:border-brand-500"
          ...
          <button
            type="button"
            className="shrink-0 rounded-lg border border-surface-muted px-3 py-1.5 text-xs font-semibold text-brand-400 outline-none"
