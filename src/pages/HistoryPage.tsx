        ...
        {filteredMatches.length ? (
          filteredMatches.map((match) => (
            <article
              key={match.id}
              className={[
                'rounded-2xl bg-surface-elevated p-4',
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-text-secondary">
                    完成：{formatDateTime(match.finishedAt)}
                  </p>
                </div>
                {match.deletedAt ? (
                  <span className="rounded-full bg-danger/15 px-3 py-1 text-xs text-red-100">
                    已刪除
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-3 text-sm text-text-secondary">
                <p>
                  先攻：
                  {match.firstPlayerId ? getPlayerName(players, match.firstPlayerId) : '未記錄'}
                </p>
                {match.notes ? <p className="mt-1">備註：{match.notes}</p> : null}
              </div>
