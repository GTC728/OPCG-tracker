...
| **9C** | PIN Cloud Backup / Restore | ✅ Part 1 完成 | Supabase 共享 PIN、手動上傳、共享資料下載、RLS schema |
...
4. 重啟 dev server。
5. 在 `設定 → App 資料 → Cloud Backup` 輸入共享 PIN。第一次使用該 PIN 時，上傳本機資料即可建立共享資料；其他朋友輸入同一 PIN 後可下載同一份資料。

共享 PIN 安全提示：PIN 會先在瀏覽器 hash，雲端不保存原文 PIN。但知道同一 PIN 的人都可讀寫共享資料，建議使用不容易猜的字串。
...
- **9C PIN Cloud Backup / Restore**：已完成第一階段。不用登入；同一組 PIN 連到同一份共享資料，可手動上傳本機資料或下載最新共享資料。
- **9D Auto Sync**：下一步。每次修改後自動同步，並處理手機和電腦同時修改的衝突。

## 目前進度

**V1.3 Data Foundation + 9A/9B/9C Part 1 長期使用基礎已完成**：資料模型已從單一 `decks` 走向 `Leader + DeckVariant`；PWA manifest 和 service worker 已建立；主要儲存已遷移到 Dexie / IndexedDB；PIN Cloud Backup / Restore 已接 Supabase。
