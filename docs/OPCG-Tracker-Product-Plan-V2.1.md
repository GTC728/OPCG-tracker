...
### 場景 F：卡店 16 人瑞士輪店賽（未來 Event Mode）

**情境**：卡店或主辦者想在店內 run 一次 16 人小型瑞士輪。店家通常會先抽籤決定第一輪走線，之後按每輪結果生成下一輪 pairing，並在活動結束後查看 standings 和報告。

**流程**：建立 Event → 選擇 Swiss → 加入參賽玩家與 Leader → 抽籤第一輪 → 輸入結果 → 自動生成下一輪 → 顯示 standings → 完成並匯出報告。

**成功標準**：店員可以在手機上快速操作，不需要把店賽混入普通 Session；pairings、結果、standings 清晰可追溯。

---

### 場景 G：OP17 到 OP19 Leader 計分賽（未來 League Mode）

**情境**：朋友群想用自訂規則長期測試環境，例如以 Leader 為單位計分，贏 1 分、輸 0 分，達 5 分後最高分 5 個 Leader 成為 Top Tier；打贏 Top Tier 得 2 分，Top Tier 輸則扣 1 分，直到 OP19 前結算。

**流程**：建立 League → 設定 date range / source filter → 設定 scoring rules → app 從既有 matches 計算 leaderboard → 顯示 Top Tier、分數來源、被 ban / restricted 的 Leader。

**成功標準**：League 不修改原始對局資料，只讀取有用的 match data 形成 scoring view；每個分數都能解釋來源。
...
| 完整官方賽事系統 | 初期專注對局記錄；未來可做 Event / League 模組，但不急於取代正式 tournament software |
...
## 16.2 Future Module: Events And Leagues

This section records the post-V3 product direction discussed after the original V2.1 plan. The detailed blueprint lives in [`EVENTS-AND-LEAGUES.md`](EVENTS-AND-LEAGUES.md).

### Product Principle

Events and Leagues should be integrated into OPCG Tracker, but they should not contaminate ordinary Session data.

- `Session`: everyday match recording for store play, testing, and casual games.
- `Event`: bounded tournament-like activity with entrants, rounds, pairings, standings, and reports.
- `League`: long-running custom ruleset that reads existing matches and calculates a leaderboard without changing the original match data.

### Store Tournament Mode

Store Tournament Mode should support small store events such as a 16-player Swiss tournament.

First practical version:

1. Create Event.
2. Choose Swiss.
3. Add entrants and Leaders.
4. Randomize round 1.
5. Enter results.
6. Generate next round pairings by score.
7. Avoid repeat pairings where possible.
8. Show standings.
9. Export event report.

This should reuse existing players, Leaders, and match recording while adding event-only context such as round number, table number, pairing, event result, and standings.

### League / Ruleset Mode

League Mode should support long-running custom scoring. Example OP17-to-OP19 Leader League:

- Unit: Leader.
- Win: 1 point.
- Loss: 0 points.
- From 5 points onward, the highest scoring 5 Leaders become Top Tier.
- Beating Top Tier gives 2 points.
- A Top Tier Leader losing costs -1 point.
- Settle standings before OP19.

The League engine should read from filtered match data:
...
The scoring output must be explainable, including base points, bonus points, penalties, Top Tier status, banned Leaders, and contributing matches.

### Ban / Restriction Rules

Ban Top Deck features should be rules inside Event or League, not a separate product surface.

Possible rules:

- Manual banned Leader list.
- Auto-ban top N Leaders.
- Top Tier threshold.
- Extra points for beating Top Tier.
- Penalty for Top Tier losing.
- Allow banned Leaders to play but not score.
- Block banned Leaders from entry.

### Future Roadmap

- V4.0: Event / League foundation.
- V4.1: Store Tournament Swiss mode.
- V4.2: League Ruleset engine.
- V4.3: Reports and sharing.
